...
@Component({
  ...
  templateUrl: './members-messages.html',
  styleUrl: './members-messages.css',
})
export class MembersMessages implements OnInit {
  ...
  loadMessages() {
    ...
    if (memberId) {
      this.messageService.getMessageTread(memberId).subscribe({
        next: (messages) =>
          this.messages.set(
